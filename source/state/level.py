...
class LevelState:
    ...
    def update(self, dt):
        ...
        # Ice + Fire bullet cancellation (steam effect) + Torchwood conversion
        ...
        for b in self.bullets[:]:
            # Torchwood: convert ice bullets to fire
        ...
        for row in range(GRID_ROWS):
            for col in range(GRID_COLS):
                ...
                # Potato mine: zombie-proximity detonation (armed at 5.0s)
                if p.name == 'potatomine' and p.armed:
                    for z in self.zombies:
                        if z.row == p.row and not z.dead:
                            if abs(z.x - p.rect.centerx) < 40:
                                p.exploding = True
                                p.explode_timer = 0.5  # trigger explode action next frame
                                break
                    if p.exploding:
                    ...
                    if p.name == 'potatomine':
                        # Potato Mine: smaller radius, triggered when armed
